...
if ($op == "add") {
	$poll_form = new icms_form_Theme(_AM_CREATNEWPOLL, "poll_form", "index.php");
	//$question_text = new icms_form_elements_Text(_AM_POLLQUESTION, "question", 50, 255);
	$question_text = new icms_form_elements_Textarea(_AM_POLLQUESTION, "question");
	...
	$multi_limit = new icms_form_elements_Text(_AM_MULTI_LIMIT . '<br/><small>' ._AM_MULTI_LIMIT_HELP . '</small>', "multilimit", 6, 5, 0);
	...
	$mail_voter_yn = new icms_form_elements_Radioyn(_AM_NOTIFY_VOTER, "mail_voter", 0);
	$poll_form->addElement($mail_voter_yn);

	$autoblockremove_yn = new icms_form_elements_Radioyn(_AM_AUTOBLOCKREMOVE, "autoblockremove", 0);
	$poll_form->addElement($autoblockremove_yn);

	$polltype_array = array (1 => _AM_POLLTYPE1, 2 => _AM_POLLTYPE2, 3 => _AM_POLLTYPE3);
	$polltype_select = new icms_form_elements_Select(_AM_POLLTYPE, "polltype", 1);
	...
	$barcolor_array = icms_core_Filesystem::getFileList(XOOPS_ROOT_PATH."/modules/umfrage/images/colorbars/");
	for ($i = 0; $i < 10; $i ++) {
		...
		$color_label = new icms_form_elements_Label("", "<img src='".XOOPS_URL."/modules/umfrage/images/colorbars/".$current_bar."' name='option_color_image[".$i."]' id='option_color_image[".$i."]' width='30' align='bottom' height='15' alt='' /><br />");
...
if ($op == "edit") {
	...
	$poll_form = new icms_form_Theme(_AM_EDITPOLL, "poll_form", "index.php");
	$author_label = new icms_form_elements_Label(_AM_AUTHOR, "<a href='".XOOPS_URL."/userinfo.php?uid=".$poll->getVar("user_id")."'>".icms_member_user_Object :: getUnameFromId($poll->getVar("user_id"))."</a>");
	...
	$question_text = new icms_form_elements_Textarea(_AM_POLLQUESTION, "question", $poll->getVar("question", "E"));
	...
	// Isegura.es: Adding "multiple selection limit" to multiple selection votes
	$multi_limit = new icms_form_elements_Text(_AM_MULTI_LIMIT . '<br/><small>' ._AM_MULTI_LIMIT_HELP . '</small>', "multilimit", 6, 5, $poll->getVar("multilimit"));
	$poll_form->addElement($multi_limit);
	...
	// ISegura.es: Adding "notify to voter" in the form
	$mail_voter_yn = new icms_form_elements_Radioyn(_AM_NOTIFY_VOTER, "mail_voter", $poll->getVar('mail_voter'));
	$poll_form->addElement($mail_voter_yn);
	
	$autoblockremove_yn = new icms_form_elements_Radioyn(_AM_AUTOBLOCKREMOVE, "autoblockremove", $poll->getVar("autoblockremove"));
	$poll_form->addElement($autoblockremove_yn);

	$polltype_array = array (1 => _AM_POLLTYPE1, 2 => _AM_POLLTYPE2, 3 => _AM_POLLTYPE3);
	$polltype_select = new icms_form_elements_Select(_AM_POLLTYPE, "polltype", $poll->getVar("polltype"));
	$polltype_select->addOptionArray($polltype_array);
	$poll_form->addElement($polltype_select);
	...
	foreach ($options_arr as $option) {
		...
		$color_select->addOptionArray($barcolor_array);
		$color_select->setExtra("onchange='showImgSelected(\"option_color_image[".$i."]\", \"option_color[".$i."]\", \"modules/umfrage/images/colorbars\")'");
		$color_label = new icms_form_elements_Label("", "<img src='".XOOPS_URL."/modules/umfrage/images/colorbars/".$option->getVar("option_color", "E")."' name='option_color_image[".$i."]' id='option_color_image[".$i."]' width='30' align='bottom' height='15' alt='' /><br />");
		$option_tray->addElement($color_select);
		$option_tray->addElement($color_label);
...
if ($op == "addmore") {
	$poll = new Umfrage($_GET['poll_id']);
	$poll_form = new icms_form_Theme(_AM_ADDMORE, "poll_form", "index.php");
...
if ($op == "restart") {
	$poll = new Umfrage($_GET['poll_id']);
	$poll_form = new icms_form_Theme(_AM_RESTARTPOLL, "poll_form", "index.php");
	...
	$poll_form->addElement($notify_yn);
	// ISegura.es: Adding "notify to voter" in the form
	$notify_voter_yn = new icms_form_elements_Radioyn(_AM_NOTIFY_VOTER, "notify_voter", 1);
	...
	if (is_array($logs_arr) && $logs_count > 0) {
		...
		for ($i = 0; $i < $max; $i ++) {
			...
			if ($logs_arr[$i]->getVar("user_id") != 0) {
				$user = new icms_member_user_Object($logs_arr[$i]->getVar("user_id"));
